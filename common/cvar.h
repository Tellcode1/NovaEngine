...
typedef enum cvar_tp
{
  CVAR_TP_INT     = 0,
  CVAR_TP_FLOAT   = 1,
  CVAR_TP_STR     = 2,
  CVAR_TP_UNKNOWN = 0x7FFFFFFF
} cvar_tp;
...
inline int
__cvar_compare(const void* a, const void* b)
{
  return ((cvar*)a)->val.d - ((cvar*)b)->val.d;
}

static inline void
cvarinit(const cvar* src)
{
  ...
  {
    nv_assert(g_nvars > 0);
    g_vars = (cvar*)nv_realloc(g_vars, g_nvars * sizeof(cvar));
  }
  g_vars[g_nvars] = var;
  g_nvars++;
}

static inline cvar_value
cvarread(const char* name)
{
...
static inline void
cvarset(const char* name, const cvar_value value)
{
  for (int i = 0; i < g_nvars; i++)
  {
    if (nv_strncmp(name, g_vars[i].name, 63) == 0)
