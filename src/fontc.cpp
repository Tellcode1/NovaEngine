...
main(int argc, char* argv[])
{
  ...
  nv_timer_t stopwatch = nv_timer_begin(__FLT_MAX__);
...
fontc_read_font(const char* path, fontc_file_t* file)
{
  ...
  bitmap_size = file->header.bmpwidth * file->header.bmpheight;
...
CLEANUP:
  ...
  if (compressed_glyphs != nullptr)
  {
    nv_free(compressed_glyphs);
  }
  if (compressed_bitmap != nullptr)
  {
...
fontc_bake_font_to_cache(const char* font_path, size_t pixel_size, size_t init_atlas_w, size_t init_atlas_h, size_t num_threads, fontc_file_t* out_file)
{
  ...
  *out_file = nv_zero_init(fontc_file_t);
...

  {
    ...
    if (buffer)
    {
      nv_image image = (nv_image){ .width = w, .height = h, .format = NOVA_FORMAT_R8, .data = (unsigned char*)buffer };
...
#pragma unroll(4)
  ...
  nv_log_info("final atlas size w=%i h=%i (uncompressed %b)\n", atlas.width, atlas.height, atlas.width * atlas.height * lr::nv_format_get_bytes_per_pixel(atlas.format));

  nv_texture_atlas_finish(&atlas);

  atlas_w = (flt_t)atlas.width;
  atlas_h = (flt_t)atlas.height;

  units_per_em = (flt_t)face->units_per_EM;
...
#pragma omp parallel for
  ...
  image_size                            = atlas.width * atlas.height * lr::nv_format_get_bytes_per_pixel(atlas.format);
