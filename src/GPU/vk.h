...
/*  */
extern void nv_vk_stage_buffer_transfer(struct nvvk_driver* driver, VkBuffer dst, void* data, size_t size);
...
extern VkCommandBuffer nv_vk_begin_command_buffer(struct nvvk_driver* driver);

/* WARNING: waitForExecution = false implies you take responsibility of freeing the commandBuffer! */
extern VkResult nv_vk_end_command_buffer(struct nvvk_driver* driver, VkCommandBuffer cmd, VkQueue queue, bool waitForExecution);

extern void nv_vk_stage_image_transfer(struct nvvk_driver* driver, VkImage dst, const void* data, size_t width, size_t height, size_t image_size);

extern void nv_vk_create_texture_from_memory(struct nvvk_driver* driver, u8* buffer, u32 width, u32 height, nv_format format, VkImage* dst, VkDeviceMemory* dstMem);

extern u8* nv_vk_create_texture_from_disk(struct nvvk_driver* driver, const char* path, u32* width, u32* height, nv_format* channels, VkImage* dst, VkDeviceMemory* dstMem);
