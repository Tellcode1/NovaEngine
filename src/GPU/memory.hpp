...
extern void nv_gpu_allocate_memory(nvvk_ctx_t* vkctx, size_t size, nv_gpu_memory_usage usage, nv_gpu_memory_t* dst);
extern void nv_gpu_free_memory(nvvk_ctx_t* vkctx, nv_gpu_memory_t* mem);

extern void nv_gpu_map_memory(nvvk_ctx_t* vkctx, nv_gpu_memory_t* memory, VkDeviceSize size, size_t offset, void** out);
extern void nv_gpu_unmap_memory(nvvk_ctx_t* vkctx, nv_gpu_memory_t* memory);

/* Warning: src must not be mapped when this function is called. */
extern void nv_gpu_copy_memory(nvvk_ctx_t* vkctx, nv_gpu_memory_t* dst, nv_gpu_memory_t* src, size_t size, size_t dst_offset, size_t src_offset);
