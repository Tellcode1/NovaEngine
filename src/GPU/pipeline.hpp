...
_nvvk_default_result_check_fn(const VkResult result, const char* file, const char* func, unsigned long line)
{
  ...
  const char* result_string = lr::nvvk_vk_result_to_string(result);
...
struct nv_gpu_pipeline_create_info
{
  ...
  const lr::nvsm_shader_t* const*          shaders;
...
namespace lr
{
extern nv_error nv_vk_bake_global_pipelines(lr::nvsm_ctx_t* ctx, nv_renderer_t* rd);
extern void     nv_vk_destroy_global_pipelines(nvvk_ctx_t* vkctx);
}

extern void nv_gpu_create_graphics_pipeline(nvvk_ctx_t* vkctx, nv_gpu_pipeline_create_info const* pCreateInfo, VkPipeline* dstPipeline, u32 flags);
extern void nv_gpu_create_depth_pipeline(nvvk_ctx_t* vkctx, nv_gpu_pipeline_create_info const* pCreateInfo, VkPipeline* dstPipeline, u32 flags);
extern void nv_gpu_create_pipeline_layout(nvvk_ctx_t* vkctx, nv_gpu_pipeline_create_info const* pCreateInfo, VkPipelineLayout* dstLayout);
extern void nv_gpu_create_render_pass(nvvk_ctx_t* vkctx, nv_gpu_render_pass_create_info const* pCreateInfo, VkRenderPass* dstRenderPass, u32 flags);
extern void nv_gpu_create_depth_pass(nvvk_ctx_t* vkctx, nv_gpu_render_pass_create_info const* pCreateInfo, VkRenderPass* dstRenderPass, u32 flags);
extern void nv_gpu_create_swapchain(nvvk_ctx_t* vkctx, nv_gpu_swapchain_create_info const* pCreateInfo, VkSwapchainKHR* dstSwapchain);
