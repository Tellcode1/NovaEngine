...
glslang_stage_t nvsm_glslang_shader_stage_from_string(const char stage[4]);

nv_error load_list_file(nvsm_ctx_t* ctx, nvsm_list_file_t* file);

/* returns NV_ERROR_IO_ERROR to indicate the file was not found. This should be handled by the dev appropriately. */
nv_error nvsm_load_cache_file(nvsm_ctx_t* ctx, nvsm_cache_file_t* file);

/* write the cache to disk */
nv_error generate_and_write_cache_file(const nvsm_list_file_t* file, size_t list_file_mtime, FILE* out_file);
...
static inline void
get_cache_file_path(const char* cache_file_dir, char buffer[256])
{
  /**
...
nv_error
load_list_file(nvsm_ctx_t* ctx, nvsm_list_file_t* file)
{
  ...
  nv_assert_else_return(list_file != NULL, NV_ERROR_IO_ERROR);

  size_t const max_line_len = get_file_max_line_length(list_file);
  ...
  size_t const num_lines_list_file = count_file_lines(list_file);
...
nv_error
nvsm_load_cache_file(nvsm_ctx_t* ctx, nvsm_cache_file_t* file)
{
  nv_assert_else_return(ctx != NULL, NV_ERROR_INVALID_ARG);
  nv_assert_else_return(file != NULL, NV_ERROR_INVALID_ARG);

  if (ctx->cache_file_dir != NULL)
  {
...
static inline void
nvsm_destroy_cache_file(nvsm_cache_file_t* file)
{
  if (file->entries != NULL)
  {
...
static inline void
destroy_list_file(nvsm_list_file_t* file)
{
  if (file->entries != NULL)
  {
    nv_assert_else_return(file->num_entries != 0, );
...
nv_error
generate_and_write_cache_file(const nvsm_list_file_t* file, size_t list_file_mtime, FILE* out_file)
{
  nv_assert_else_return(file != NULL, NV_ERROR_INVALID_ARG);
...
nvsm_dump_shader(const nvsm_spirv_binary_t* bin, const char* out_filename)
{
...
static inline nv_error
nvsm_read_file_flattened(const char* file_path, char** dst, size_t* dst_size)
{
...
glslang_stage_t
nvsm_glslang_shader_stage_from_string(const char stage[4])
{
...
static inline nv_error
nvsm_compile_shader(const char* shader_path, const nvsm_compile_options_t* opts, const char* spirv_path, const char stage[4], nvsm_spirv_binary_t* bin, bool dump)
{
  ...
  if ((code = nvsm_read_file_flattened(shader_path, &shader_source, &shader_size)) != NV_SUCCESS)
  {
    return code;
  }
  ...
  const glslang_input_t input = {
    ...
    .stage                             = nvsm_glslang_shader_stage_from_string(stage),
...
static inline nv_error
compile_no_cache(nvsm_ctx_t* ctx, nvsm_list_file_t* list_file)
{
  nvsm_compile_options_t compile_options = nvsm_get_default_compile_options();
  ...
  for (size_t list_i = 0; list_i < list_file->num_entries; list_i++)
  {
    ...
    if ((code = nvsm_compile_shader(list_entry->shader_path, &compile_options, list_entry->spirv_path, list_entry->stage, &list_entry->bin, true)) != NV_SUCCESS)
    {
      continue;
    }
...
static inline nv_error
nvsm_default_compile_with_cache(nvsm_ctx_t* ctx, nvsm_list_file_t* list_file, nvsm_cache_file_t* cache_file)
{
  nv_error code = NV_SUCCESS;
    ...
    */
    ...
    for (size_t cache_i = 0; cache_i < cache_file->num_entries; cache_i++)
    {
      ...
      if (hash_name == cache_entry->hash_name && hash_shader_path == cache_entry->hash_shader_path)
      {
        ...
        code = nvsm_compile_shader(list_entry->shader_path, &compile_options, list_entry->spirv_path, list_entry->stage, &list_entry->bin, true);
        if (code != NV_SUCCESS)
        {
 ...
 */
static inline nv_error
generate_and_dump_cache_file(const char* cache_file_dir, const nvsm_list_file_t* list_file, size_t list_file_last_modtime)
{
  /**
   * This is legal
...
nvsm_compile_shaders(nvsm_ctx_t* ctx)
{
  ...
  code = nvsm_load_cache_file(ctx, &cache_file);

  if (code == NV_ERROR_IO_ERROR || code == NV_ERROR_INVALID_CACHE)
  {
    // continue without cache
    if ((code = compile_no_cache(ctx, &list_file)) != NV_SUCCESS)
    {
      return code;
    }
  ...
  else
  {
    ...
    nvsm_destroy_cache_file(&cache_file);
  ...
  destroy_list_file(&list_file);

  return NV_SUCCESS;
}

nv_error
nvsm_create_shader_modules(nvvk_ctx_t* vkctx, nvsm_ctx_t* ctx)
{
  nv_assert_else_return(ctx != NULL, NV_ERROR_INVALID_ARG);
...
VkShaderStageFlags
nvsm_shader_stage_from_string(const char stage[4])
{
  if (nv_strncmp(stage, "vert", 4) == 0)
  {
...
nvsm_init(nvsm_ctx_t* ctx)
{
  ...
  ctx->custom_options = nvsm_get_default_compile_options();

  return NV_SUCCESS;
}

void
nvsm_shutdown(nvvk_ctx_t* vkctx, nvsm_ctx_t* ctx)
{
  ...
  while ((node = nv_hashmap_iterate_unsafe(&ctx->shader_map, &_hashmap_iter)) != NULL)
  {
    ...
    if (entry->resources != NULL && entry->num_resources > 0)
    {
      ...
      {
        nvsm_shader_resources_t* resource = &entry->resources[i];
...
nvsm_resource_type
nvsm_shader_resources_type_to_spv_reflect_type(SpvReflectDescriptorType_ type)
{
  switch (type)
  {
    case SPV_REFLECT_DESCRIPTOR_TYPE_UNIFORM_BUFFER: return NVSM_SHADER_RESOURCE_TYPE_UNIFORM_BUFFER;
    case SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_BUFFER: return NVSM_SHADER_RESOURCE_TYPE_STORAGE_BUFFER;
    case SPV_REFLECT_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER: return NVSM_SHADER_RESOURCE_TYPE_COMBINED_IMAGE_SAMPLER;
    case SPV_REFLECT_DESCRIPTOR_TYPE_SAMPLED_IMAGE: return NVSM_SHADER_RESOURCE_TYPE_SAMPLED_IMAGE;
    case SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_IMAGE: return NVSM_SHADER_RESOURCE_TYPE_STORAGE_IMAGE;
    case SPV_REFLECT_DESCRIPTOR_TYPE_SAMPLER: return NVSM_SHADER_RESOURCE_TYPE_SAMPLER;
    default: return NVSM_SHADER_RESOURCE_TYPE_UNKNOWN;
  }
}

nvsm_shader_resources_t*
nvsm_shader_resources_extract(const u32* spirv_data, size_t spirv_size, size_t* out_count)
{
  ...
  SpvReflectResult const result = spvReflectCreateShaderModule(spirv_size, spirv_data, &handle);
  ...
  for (size_t i = 0; i < binding_count; i++)
  {
    ...
    resources[i].buffer_size  = binding->block.padded_size;
  }

  *out_count = (size_t)binding_count;

  nv_free((void*)bindings);
  spvReflectDestroyShaderModule(&handle);
  return resources;
}