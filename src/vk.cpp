...
{
  nv_rdr_per_image_data_t* access = (nv_rdr_per_image_data_t*)nv_list_get(&rd->per_image_data, rd->image_index);
  if (NV_LIKELY(access))
  {
    return access->cmd;
  }
...
nv_renderer_prepare_quad_renderer(nv_renderer_t* rd)
{
  ...
  nv_error code = nv_gpu_memory_pool_allocate(&rd->driver->gpu_local_pool, sizeof(quad_vertices) + sizeof(quad_indices), 4, &rd->quad_memory);
  nv_assert_else_return(code == NV_SUCCESS, code);

  nv_gpu_buffer_init(
      rd->driver, &rd->quad_memory, sizeof(quad_vertices) + sizeof(quad_indices), 1, NV_GPU_BUFFER_VERTEX_BUFFER_BIT | NV_GPU_BUFFER_INDEX_BUFFER_BIT, &rd->quad_vb);
  nv_assert_else_return(code == NV_SUCCESS, code);
...
nv_renderer_destroy(nv_renderer_t* rd)
{
  ...
  for (size_t i = 0; i < rd->swap_chain_image_count; i++)
  {
    ...
    vkDestroyImageView(rd->vkctx->device, nv_gpu_texture_get_view(&data->sc_image), &rd->vkctx->vkalloc);

    nv_gpu_destroy_texture(rd->vkctx, &data->depth_image);

    nv_gpu_destroy_framebuffer(rd->vkctx, &data->color_framebuffer);
  }

  for (size_t i = 0; i < nv_list_size(&rd->per_frame_data); i++)
  {
    nv_rdr_per_frame_data_t* image_frame = (nv_rdr_per_frame_data_t*)nv_list_get(&rd->per_frame_data, i);
  ...
  for (size_t i = 0; i < nv_list_size(&rd->per_image_data); i++)
  {
    nv_rdr_per_image_data_t* image_frame = (nv_rdr_per_image_data_t*)nv_list_get(&rd->per_image_data, i);
  ...
  nv_camera_destroy(rd->vkctx, &camera);

  lr::nv_vk_destroy_global_pipelines(rd->vkctx);
  nv_descriptor_pool_destroy(rd->vkctx, &g_pool);

  ctext_shutdown(rd);

  nv_gpu_free_memory(rd->vkctx, &rd->depth_image_memory);

  nv_gpu_buffer_destroy(&rd->quad_vb);
  nv_gpu_memory_pool_free(&rd->quad_memory);

  VkCommandBuffer buffer[32];
  for (size_t i = 0; i < nv_list_size(&rd->per_image_data); i++)
  {
    nv_rdr_per_image_data_t* frame = (nv_rdr_per_image_data_t*)nv_list_get(&rd->per_image_data, i);
    buffer[i]                      = frame->cmd;
  }

  vkFreeCommandBuffers(rd->vkctx->device, rd->command_pool, nv_list_size(&rd->per_image_data), buffer);
  vkDestroyCommandPool(rd->vkctx->device, rd->command_pool, &rd->vkctx->vkalloc);

  vkFreeCommandBuffers(rd->vkctx->device, rd->vkctx->cmd_pool, NOVA_GPU_COMMAND_BUFFER_CACHE_COUNT, rd->vkctx->cmd_buffers);
  vkDestroyCommandPool(rd->vkctx->device, rd->vkctx->cmd_pool, &rd->vkctx->vkalloc);

  vkDestroySwapchainKHR(rd->vkctx->device, rd->swapchain, &rd->vkctx->vkalloc);

  if (rd->flags & NOVA_RENDERER_MULTISAMPLING_ENABLE)
  {
    nv_gpu_destroy_texture(rd->vkctx, &rd->color_image);
    nv_gpu_free_memory(rd->vkctx, &rd->color_image_memory);
  }
  ...
  nv_list_destroy(&rd->per_frame_data);
  nv_list_destroy(&rd->per_image_data);
...
create_optional_images(nv_renderer_t* rd)
{
  ...
  if (nvvk_result_check(*rd->vkctx, vkGetSwapchainImagesKHR(rd->vkctx->device, rd->swapchain, &rd->swap_chain_image_count, NULL)) != VK_SUCCESS)
  {
  ...
  if (nvvk_result_check(*rd->vkctx, vkGetSwapchainImagesKHR(rd->vkctx->device, rd->swapchain, &rd->swap_chain_image_count, swapchainImages)) != VK_SUCCESS)
  {
    return NV_ERROR_INVALID_RETVAL;
  }

  nv_list_reserve(&rd->render_data, rd->swap_chain_image_count);
  nv_list_clear(&rd->render_data);

  if (rd->flags & NOVA_RENDERER_MULTISAMPLING_ENABLE)
  {
    size_t                     color_image_size = 0;
    nv_gpu_texture_create_info color_image_desc = {
      .format      = rd->swap_chain_image_format,
      .samples     = (VkSampleCountFlagBits)rd->samples,
      .type        = VK_IMAGE_TYPE_2D,
      .usage       = NOVA_GPU_TEXTURE_USAGE_RESOLVE_TEXTURE,
      .extent      = (nv_extent3D){ rd->render_extent.width, rd->render_extent.height, 1 },
      .arraylayers = 1,
      .miplevels   = 1,
    };
    nv_gpu_create_texture(rd->vkctx, &color_image_desc, &rd->color_image);

    nv_gpu_allocate_memory(rd->vkctx, color_image_size, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT, &rd->color_image_memory);
    nv_gpu_bind_texture_to_memory(rd->vkctx, &rd->color_image_memory, 0, &rd->color_image);
  }
  ...
  for (size_t i = 0; i < rd->swap_chain_image_count; i++)
  {
    ...
    nv_gpu_create_texture(rd->vkctx, &image_info, &data->depth_image);
    ...
    if (i == 0)
    {
      ...
      vkGetImageMemoryRequirements(rd->vkctx->device, nv_gpu_texture_get(&data->depth_image), &memReqs);

      rd->shadow_image_size = memReqs.size;
      nv_assert_else_return(rd->shadow_image_size != 0, NV_ERROR_INVALID_RETVAL);

      nv_gpu_allocate_memory(rd->vkctx, rd->shadow_image_size * rd->swap_chain_image_count, NOVA_GPU_MEMORY_USAGE_GPU_LOCAL, &rd->depth_image_memory);
      nv_assert_else_return(rd->depth_image_memory.memory != VK_NULL_HANDLE, NV_ERROR_INVALID_RETVAL);
    }

    nv_gpu_bind_texture_to_memory(rd->vkctx, &rd->depth_image_memory, i * rd->shadow_image_size, &data->depth_image);
  }
...
create_framebuffers_and_swapchain_image_views(nv_renderer_t* rd)
{
  ...
  for (size_t i = 0; i < rd->swap_chain_image_count; i++)
  {
    ...
    imageViewCreateInfo.format                          = (VkFormat)lr::nv_format_to_vk_format(rd->swap_chain_image_format);
    ...
    if (nv_gpu_create_framebuffer(rd->vkctx, &framebuffer_info, &data->color_framebuffer) != 0)
    {
...
nv_renderer_initialize_rendering_components(nv_renderer_t* rd, const nv_renderer_config* conf)
{
  ...
  swapchain_create_info.color_space                  = (VkColorSpaceKHR)rd->swap_chain_color_space;
  swapchain_create_info.image_count                  = rd->swap_chain_image_count;

  nv_gpu_create_swapchain(rd->vkctx, &swapchain_create_info, &rd->swapchain);
  ...
  const size_t frames_in_flight = nv_renderer_get_max_frames_in_flight(rd);
  ...
  cmdAllocInfo.commandBufferCount          = rd->swap_chain_image_count;
  ...
  nv_list_resize(&rd->per_image_data, rd->swap_chain_image_count);
  for (size_t i = 0; i < rd->swap_chain_image_count; i++)
  {
  ...
  rd->depth_buffer_format = (VkFormat)lr::nv_format_to_vk_format(NOVA_FORMAT_D32); // replace (probably)
  ...
  rpi.depth_buffer_format = lr::nv_vk_format_to_nv_format(rd->depth_buffer_format);
  ...
  nv_gpu_create_render_pass(rd->vkctx, &rpi, &rd->render_pass, rd->vkctx->flag_register);
...
nv_error
nv_renderer_init(struct nv_ctx* ctx, lr::nvsm_ctx_t* nvsmctx, nvvk_driver* driver, const nv_renderer_config* conf, nv_renderer_t* dst)
{
  nv_assert_else_return(conf != NULL, NV_ERROR_INVALID_ARG);
  ...
  dst->swap_chain_image_count = lr::nv_vk_get_surface_image_count(dst->vkctx, dst->vkctx->phys_device, dst->vkctx->surface);
  ...
  if (!lr::nv_vk_get_supported_format(dst->vkctx, dst->vkctx->phys_device, dst->vkctx->surface, &dst->swap_chain_image_format, (VkColorSpaceKHR*)&dst->swap_chain_color_space))
  {
  ...
  code = nv_list_init(sizeof(nv_renderer_frame_render_info), dst->swap_chain_image_count, nv_allocator_c, NULL, &dst->render_data);
  nv_assert_else_return(code == NV_ERROR_SUCCESS, code);

  code = nv_list_init(sizeof(nv_rdr_per_frame_data_t), nv_renderer_get_max_frames_in_flight(dst), nv_allocator_c, NULL, &dst->per_frame_data);
  nv_assert_else_return(code == NV_ERROR_SUCCESS, code);

  code = nv_list_init(sizeof(nv_rdr_per_image_data_t), dst->swap_chain_image_count, nv_allocator_c, NULL, &dst->per_image_data);
  nv_assert_else_return(code == NV_ERROR_SUCCESS, code);
  ...
  nv_assert_else_return(nv_list_is_valid(&dst->per_frame_data), NV_ERROR_BROKEN_STATE);
  ...
  nv_list_resize(&dst->per_frame_data, nv_renderer_get_max_frames_in_flight(dst));
  nv_list_resize(&dst->per_image_data, dst->swap_chain_image_count);

  for (size_t i = 0; i < dst->swap_chain_image_count; i++)
  {
    nv_rdr_per_image_data_t* image_frame = (nv_rdr_per_image_data_t*)nv_list_get(&dst->per_image_data, i);
  ...
  for (size_t i = 0; i < nv_list_size(&dst->per_frame_data); i++)
  {
    nv_rdr_per_frame_data_t* frame = (nv_rdr_per_frame_data_t*)nv_list_get(&dst->per_frame_data, i);
  ...
  if ((code = lr::nv_vk_bake_global_pipelines(nvsmctx, dst)) != NV_ERROR_SUCCESS)
  {
    return code;
  }
...
_nvvk_renderer_resize(nv_renderer_t* rd)
{
  ...
  for (size_t i = 0; i < nv_list_size(&rd->per_frame_data); i++)
  {
    nv_rdr_per_frame_data_t* frame = (nv_rdr_per_frame_data_t*)nv_list_get(&rd->per_frame_data, i);
    if (!frame)
    {
      continue;
    }
  ...
  if (rd->depth_image_memory.memory != VK_NULL_HANDLE)
  {
    nv_gpu_free_memory(rd->vkctx, &rd->depth_image_memory);
  }
  if (rd->color_image.image != VK_NULL_HANDLE)
  {
    nv_gpu_destroy_texture(rd->vkctx, &rd->color_image);
    nv_gpu_free_memory(rd->vkctx, &rd->color_image_memory);
  }
  for (size_t i = 0; i < nv_list_size(&rd->render_data); i++)
  {
    nv_renderer_frame_render_info* data = (nv_renderer_frame_render_info*)nv_list_get(&rd->render_data, i);
    if (!data)
    {
    ...
    if (data->depth_image.image != VK_NULL_HANDLE)
    {
      nv_gpu_destroy_texture(rd->vkctx, &data->depth_image);
    }
    if (data->sc_image.image != VK_NULL_HANDLE)
    {
      vkDestroyImageView(rd->vkctx->device, nv_gpu_texture_get_view(&data->sc_image), &rd->vkctx->vkalloc);
  ...
  rd->swap_chain_image_count = lr::nv_vk_get_surface_image_count(rd->vkctx, rd->vkctx->phys_device, rd->vkctx->surface);
  int w, h;
  ...
  nv_list_clear(&rd->per_frame_data);
  nv_list_clear(&rd->per_image_data);

  nv_list_resize(&rd->per_frame_data, nv_renderer_get_max_frames_in_flight(rd));
  nv_list_resize(&rd->per_image_data, rd->swap_chain_image_count);
  ...
  size_t frames_in_flight = nv_renderer_get_max_frames_in_flight(rd);
  ...
  cmdAllocInfo.commandBufferCount          = rd->swap_chain_image_count;
  cmdAllocInfo.commandPool                 = rd->command_pool;
  VkResult allocResult                     = vkAllocateCommandBuffers(rd->vkctx->device, &cmdAllocInfo, buffer);
...
nv_renderer_begin(nv_renderer_t* rd, vec4 clear_color)
{
  ...
  // nv_rdr_per_image_data_t* image_frame = (nv_rdr_per_image_data_t*)nv_list_get(&rd->per_image_data, rd->frame);
  nv_rdr_per_frame_data_t* frame = (nv_rdr_per_frame_data_t*)nv_list_get(&rd->per_frame_data, rd->frame);

  // Check if we need to resize
  bool needs_resize = frame == NULL || frame->in_flight_fence == VK_NULL_HANDLE;
  if (needs_resize)
  {
  ...
  VkResult waitResult = vkWaitForFences(rd->vkctx->device, 1, &frame->in_flight_fence, VK_TRUE, UINT64_MAX);
  ...
  VkResult image_acquire_res = vkAcquireNextImageKHR(rd->vkctx->device, rd->swapchain, UINT64_MAX, frame->image_available_semaphore, VK_NULL_HANDLE, &rd->image_index);

  if (image_acquire_res == VK_ERROR_OUT_OF_DATE_KHR || image_acquire_res == VK_SUBOPTIMAL_KHR || nv_get_frame_buffer_resized(rd->ctx))
  {
    _nvvk_renderer_resize(rd);
    return false;
  }
  ...
  nv_rdr_per_image_data_t* image_frame = (nv_rdr_per_image_data_t*)nv_list_get(&rd->per_image_data, rd->image_index);
  if (image_frame == NULL)
  {
    _nvvk_renderer_resize(rd);
    return nv_renderer_begin(rd, clear_color);
  ...
  nv_renderer_frame_render_info* image = (nv_renderer_frame_render_info*)nv_list_get(&rd->render_data, rd->image_index);
  if (image == NULL)
  {
    _nvvk_renderer_resize(rd);
    return nv_renderer_begin(rd, clear_color);
  ...
  clear_values[0].color        = (VkClearColorValue){ { clear_color.x, clear_color.y, clear_color.z, clear_color.w } };
  clear_values[1].depthStencil = (VkClearDepthStencilValue){ 1.0f, 0 };

  VkRect2D render_extent = { .offset = { 0, 0 }, .extent = { (uint32_t)rd->render_extent.width, (uint32_t)rd->render_extent.height } };
...
nv_renderer_end(nv_renderer_t* rd)
{
  ...
  if (rd->will_render_this_frame)
  {
    ...
    {
      ctext_label_t*           label  = (ctext_label_t*)nv_list_get(&rd->ctext->labels, i);
  ...
  VkResult submitResult = vkQueueSubmit(rd->vkctx->graphics_queue, 1, &submit_info, frame->in_flight_fence);
  ...
  VkResult presentResult = vkQueuePresentKHR(rd->vkctx->present_queue, &present_info);
  if (presentResult == VK_ERROR_OUT_OF_DATE_KHR || presentResult == VK_SUBOPTIMAL_KHR || nv_get_frame_buffer_resized(rd->ctx))
  {
    nv_error code = NV_ERROR_SUCCESS;
  ...
  // Update frame counter for the next frame
...
nvvk_setup_debug_messenger(nvvk_ctx_t* vkctx)
{
  ...
  PFN_vkCreateDebugUtilsMessengerEXT _CreateDebugUtilsMessenger = (PFN_vkCreateDebugUtilsMessengerEXT)vkGetInstanceProcAddr(vkctx->instance, "vkCreateDebugUtilsMessengerEXT");
  ...
  VkResult r;
  if ((r = _CreateDebugUtilsMessenger(vkctx->instance, &create_info, &vkctx->vkalloc, &vkctx->debug_messenger)) != VK_SUCCESS)
  {
    nv_log_error("Vulkan debug messenger could not start. err %i\n", r);
    return;
  }

  VkDebugUtilsMessengerCallbackDataEXT tmp_message = (VkDebugUtilsMessengerCallbackDataEXT){
...
_nvvk_choose_physical_device(nvvk_ctx_t* vkctx, VkInstance instance, VkSurfaceKHR surface)
{
...
static inline nv_error
_nvvk_ctx_setup_queues(nvvk_ctx_t* vkctx)
{
  u32 queue_count = 0;
  vkGetPhysicalDeviceQueueFamilyProperties(vkctx->phys_device, &queue_count, NULL);
...
static inline void
_ctext_load_font_upload_glyph_atlas(nvvk_ctx_t* vkctx, nv_renderer_t* rd, const nv_texture_atlas_t* atlas, cfont_t* dst)
{
  ...
  nv_gpu_create_texture(rd->vkctx, &image_info, &dst->texture);

  VkMemoryRequirements imageMemoryRequirements;
  vkGetImageMemoryRequirements(vkctx->device, nv_gpu_texture_get(&dst->texture), &imageMemoryRequirements);

  nv_gpu_allocate_memory(vkctx, imageMemoryRequirements.size, NOVA_GPU_MEMORY_USAGE_GPU_LOCAL, &dst->texture_mem);
  nv_gpu_bind_texture_to_memory(rd->vkctx, &dst->texture_mem, 0, &dst->texture);

  const size_t atlas_w = atlas->width;
  const size_t atlas_h = atlas->height;

  nv_image atlas_img = (nv_image){ .width = atlas_w, .height = atlas_h, .format = NOVA_FORMAT_R8, .data = (unsigned char*)atlas->data };
...
static inline void
_ctext_load_font_update_descriptors(nvvk_ctx_t* vkctx, nv_ctext_module* ctext, cfont_t* dst)
{
...
ctext_load_font(nvvk_ctx_t* vkctx, nv_renderer_t* rdr, const char* font_path, int scale, cfont_t* dst)
{
  ...
  *(cfont_t**)nv_list_push_empty(&rdr->ctext->fonts) = dst;
  ...
  _ctext_load_font_upload_glyph_atlas(vkctx, rdr, &atlas, dst);
  _ctext_load_font_update_descriptors(vkctx, rdr->ctext, dst);

  fontc_clean_font_file(&f_file);

  const size_t initial_font_gpu_buffer_size = 1024;

  nv_error code = nv_gpu_memory_pool_allocate(&rdr->driver->gpu_local_pool, initial_font_gpu_buffer_size, 4, &dst->buffer_mem);
  nv_assert_else_return(code == NV_SUCCESS, );

  code = nv_gpu_memory_pool_allocate(&rdr->driver->cpu_mappable_pool, initial_font_gpu_buffer_size, 4, &dst->staging_buffer_mem);
  nv_assert_else_return(code == NV_SUCCESS, );

  code = nv_gpu_buffer_init(rdr->driver, &dst->buffer_mem, initial_font_gpu_buffer_size, 4, NV_GPU_BUFFER_VERTEX_BUFFER_BIT | NV_GPU_BUFFER_INDEX_BUFFER_BIT, &dst->buffer);
  nv_assert_else_return(code == NV_SUCCESS, );

  code = nv_gpu_buffer_init(rdr->driver, &dst->staging_buffer_mem, initial_font_gpu_buffer_size, 4, NV_GPU_BUFFER_TRANSFER_ONLY_BIT, &dst->staging_buffer);
  nv_assert_else_return(code == NV_SUCCESS, );
...
ctext_destroy_font(nvvk_ctx_t* vkctx, cfont_t* fnt)
{
  ...
  nv_gpu_destroy_texture(vkctx, &fnt->texture);
  nv_gpu_free_memory(vkctx, &fnt->texture_mem);

  if (fnt->buffer.buffer != VK_NULL_HANDLE)
  {
    // nv_gpu_destroy_buffer(vkctx, &fnt->buffer);
    nv_gpu_buffer_destroy(&fnt->buffer);
    // nv_gpu_free_memory(vkctx, &fnt->buffer_mem);
  }
...
static inline bool
_ctext_font_resize_buffer(nvvk_ctx_t* vkctx, cfont_t* fnt, size_t new_buffer_size)
{
  if (ctext_validate_font(fnt) != 0)
  {
    // nv_log_error("Broken font\n");
  ...
  size_t allocated_size      = nv_gpu_buffer_size(&fnt->buffer);

  if (allocated_size < new_buffer_size)
  {
    new_allocation_size = NV_MAX(allocated_size * 2, new_buffer_size);
  }
  else if (new_buffer_size < (allocated_size / 3))
  {
    new_allocation_size = NV_MAX(allocated_size / 3, new_buffer_size);
  }
  else
  {
    return false;
  ...
  nvvk_driver* driver = fnt->rd->driver;

  code = nv_gpu_memory_pool_allocate(&driver->gpu_local_pool, new_allocation_size, 4, &fnt->buffer_mem);
  nv_assert_else_return(code == NV_SUCCESS, code);

  code = nv_gpu_memory_pool_allocate(&driver->cpu_mappable_pool, new_allocation_size, 4, &fnt->staging_buffer_mem);
  nv_assert_else_return(code == NV_SUCCESS, code);

  code = nv_gpu_buffer_init(fnt->rd->driver, &fnt->buffer_mem, new_allocation_size, 4, NV_GPU_BUFFER_VERTEX_BUFFER_BIT | NV_GPU_BUFFER_INDEX_BUFFER_BIT, &fnt->buffer);
  nv_assert_else_return(code == NV_SUCCESS, false);

  code = nv_gpu_buffer_init(fnt->rd->driver, &fnt->staging_buffer_mem, new_allocation_size, 4, NV_GPU_BUFFER_TRANSFER_ONLY_BIT, &fnt->staging_buffer);
  nv_assert_else_return(code == NV_SUCCESS, false);
...
ctext_render(cfont_t* fnt, const ctext_text_render_info_t* pInfo, const char* fmt, ...)
{
  ...
  if (buffer_size < 1024)
  {
    buffer = (char*)nv_malloc(buffer_size + 1);
  }
  else
  {
    buffer = stack;
...
_ctext_upload_vertices_and_render_drawcalls(nv_renderer_t* rd, cfont_t* fnt)
{
  ...
  const bool fnt_buffer_resized = _ctext_font_resize_buffer(rd->vkctx, fnt, total_buffer_size);
...
int
_nv_descriptor_pool_allocate(nvvk_ctx_t* vkctx, nv_descriptor_pool_t* pool)
{
...
nv_descriptor_pool_init(nvvk_ctx_t* vkctx, nv_descriptor_pool_t* dst)
{
  *dst                                 = nv_zero_init(nv_descriptor_pool_t);
  ...
  dst->nsets          = 0;
  if (_nv_descriptor_pool_allocate(vkctx, dst) != 0)
  {
    return -1;
  }
  return 0;
...
nv_allocate_descriptor_set(nvvk_ctx_t* vkctx, nv_descriptor_pool_t* pool, const VkDescriptorSetLayoutBinding* bindings, int nbindings, nv_descriptor_set_t** dst)
{
  ...
  if (need_realloc || ((pool->nsets + 1) > pool->max_child_sets))
  {
    ...
    {
      pool->max_child_sets = NV_MAX(pool->max_child_sets * 2, 1);
      pool->sets           = (nv_descriptor_set_t**)nv_realloc(pool->sets, pool->max_child_sets * sizeof(nv_descriptor_set_t));
      nv_assert_else_return(pool->sets != NULL, NV_ERROR_MALLOC_FAILED);
    }

    _nv_descriptor_pool_allocate(vkctx, pool);
  }
...
static inline nv_error
__BakeUnlitPipeline(lr::nvsm_ctx_t* ctx, nv_renderer_t* rd)
{
  ...
  lr::nvsm_shader_t *vertex, *fragment;
  nv_assert_else_return(lr::nvsm_load_shader(ctx, "Unlit/vert", &vertex) == 0, NV_ERROR_EXTERNAL);
  nv_assert_else_return(lr::nvsm_load_shader(ctx, "Unlit/frag", &fragment) == 0, NV_ERROR_EXTERNAL);

  nv_assert(vertex != NULL && fragment != NULL);

  const lr::nvsm_shader_t*    shaders[] = { vertex, fragment };
  ...
  pc.shaders   = (lr::nvsm_shader_t* const*)shaders;
  ...
  nv_gpu_create_pipeline_layout(rd->vkctx, &pc, &g_Pipelines.unlit.pipeline_layout);
  pc.pipeline_layout = g_Pipelines.unlit.pipeline_layout;
  nv_gpu_create_graphics_pipeline(rd->vkctx, &pc, &g_Pipelines.unlit.pipeline, 0);

  return NV_ERROR_SUCCESS;
}

static inline nv_error
__BakeCtextPipeline(lr::nvsm_ctx_t* ctx, nv_renderer_t* rd)
{
  const VkVertexInputAttributeDescription attributeDescriptions[] = {
    // location; binding; format; offset;
    { 0, 0, (VkFormat)lr::nv_format_to_vk_format(NOVA_FORMAT_RGB32), 0 },            // pos
    { 1, 0, (VkFormat)lr::nv_format_to_vk_format(NOVA_FORMAT_RG32), sizeof(vec3f) }, // uv
  };
  ...
  lr::nvsm_shader_t*    shaders[] = { vertex, fragment };
  ...
  pc.n_shaders = nv_arrlen(shaders);
  pc.shaders   = (lr::nvsm_shader_t**)shaders;
  ...
  nv_gpu_create_pipeline_layout(rd->vkctx, &pc, &g_Pipelines.ctext.pipeline_layout);
  pc.pipeline_layout = g_Pipelines.ctext.pipeline_layout;
  nv_gpu_create_graphics_pipeline(rd->vkctx, &pc, &g_Pipelines.ctext.pipeline, 0);

  return NV_ERROR_SUCCESS;
}

static inline nv_error
__BakeDebugLinePipeline(lr::nvsm_ctx_t* ctx, nv_renderer_t* rd)
{
  struct line_push_constants
  {
    mat4f model;
  ...
  pc.n_shaders = nv_arrlen(shaders);
  pc.shaders   = (const lr::nvsm_shader_t**)shaders;
  ...
  nv_gpu_create_pipeline_layout(rd->vkctx, &pc, &g_Pipelines.line.pipeline_layout);
  pc.pipeline_layout = g_Pipelines.line.pipeline_layout;
  nv_gpu_create_graphics_pipeline(rd->vkctx, &pc, &g_Pipelines.line.pipeline, 0);

  return NV_ERROR_SUCCESS;
}

nv_error
lr::nv_vk_bake_global_pipelines(lr::nvsm_ctx_t* ctx, nv_renderer_t* rd)
{
  nv_assert_else_return(rd != NULL, NV_ERROR_INVALID_ARG);
  nv_assert_else_return(nvvk_ctx_is_valid(rd->vkctx) == true, NV_ERROR_INVALID_ARG);
  nv_assert_else_return(rd->render_extent.width != 0, NV_ERROR_BROKEN_STATE);
...
void
lr::nv_vk_destroy_global_pipelines(nvvk_ctx_t* vkctx)
{
...
void
nv_gpu_create_graphics_pipeline(nvvk_ctx_t* vkctx, const nv_gpu_pipeline_create_info* pCreateInfo, VkPipeline* dstPipeline, u32 flags)
{
  ...
  {
    if (!pCreateInfo->shaders[i] || pCreateInfo->shaders[i]->handle == VK_NULL_HANDLE)
    {
      continue;
    }

    const VkShaderStageFlagBits stage = (VkShaderStageFlagBits)lr::_nvsm_shader_stage_from_string(pCreateInfo->shaders[i]->stage);
    if ((int)stage == -1)
    {
      nv_log_error("Error in parsing stage for shader %i(%p): \"%s\"", i, pCreateInfo->shaders[i], pCreateInfo->shaders[i]->stage);
      continue;
    }
...
void
nv_gpu_create_render_pass(nvvk_ctx_t* vkctx, nv_gpu_render_pass_create_info const* pCreateInfo, VkRenderPass* dstRenderPass, u32 flags)
{
  NVVK_REQUIRED_PTR(vkctx->device);
  NVVK_REQUIRED_PTR(pCreateInfo);
  ...
  VkAttachmentDescription colorAttachmentDescription = {
    .flags          = 0,
    .format         = (VkFormat)lr::nv_format_to_vk_format(pCreateInfo->format),
  ...
  if (HAS_FLAG(NVVK_PIPELINE_FLAGS_FORCE_DEPTH_CHECK))
  {
    depthAttachment = (VkAttachmentDescription){
      .flags          = 0,
      .format         = (VkFormat)lr::nv_format_to_vk_format(pCreateInfo->depth_buffer_format),
...
void
nv_gpu_create_pipeline_layout(nvvk_ctx_t* vkctx, nv_gpu_pipeline_create_info const* pCreateInfo, VkPipelineLayout* dstLayout)
{
  NVVK_REQUIRED_PTR(vkctx->device);
  NVVK_REQUIRED_PTR(pCreateInfo);
...
void
nv_gpu_create_swapchain(nvvk_ctx_t* vkctx, nv_gpu_swapchain_create_info const* pCreateInfo, VkSwapchainKHR* dstSwapchain)
{
  NVVK_REQUIRED_PTR(vkctx->device);
  NVVK_REQUIRED_PTR(pCreateInfo);
  NVVK_NOT_EQUAL_TO(pCreateInfo->extent.width, 0);
  ...
  VkSurfaceFormatKHR surface_format = (VkSurfaceFormatKHR){ (VkFormat)lr::nv_format_to_vk_format(pCreateInfo->format), pCreateInfo->color_space };
  ...
  for (u32 i = 0; i < surface_format_count; i++)
  {
    ...
    const VkFormat            vk_format         = (VkFormat)lr::nv_format_to_vk_format(pCreateInfo->format);
    ...
    nvvk_ctx_t* vkctx, size_t size, VkBufferUsageFlags usageFlags, VkMemoryPropertyFlags propertyFlags, VkBuffer* dstBuffer, VkDeviceMemory* retMem, bool externallyAllocated)
{
  ...
  if (!externallyAllocated)
  {
    ...
    allocInfo.memoryTypeIndex      = (uint32_t)lr::nv_vk_get_mem_type(vkctx, bufferMemoryRequirements.memoryTypeBits, propertyFlags);
...
u32
lr::nv_vk_get_mem_type(nvvk_ctx_t* vkctx, const u32 memoryTypeBits, const VkMemoryPropertyFlags memoryProperties)
{
...
VkCommandBuffer
lr::nv_vk_begin_command_buffer_from(VkCommandBuffer src)
{
...
static inline void
_nv_update_command_buffers_status(nvvk_ctx_t* vkctx)
{
...
VkCommandBuffer
lr::nv_vk_begin_command_buffer(nvvk_driver* driver)
{
  ...
  if (vkctx->cmd_pool == VK_NULL_HANDLE)
  {
    ...
    for (VkFence& cmd_buffer_fence : vkctx->cmd_buffer_fences)
    {
      nvvk_result_check(*vkctx, vkCreateFence(vkctx->device, &fence_create_info, &vkctx->vkalloc, &cmd_buffer_fence));
    }
  }

  _nv_update_command_buffers_status(vkctx);
  ...
  return lr::nv_vk_begin_command_buffer_from(free_cmd_buffer);
}

VkResult
lr::nv_vk_end_command_buffer(nvvk_driver* driver, VkCommandBuffer cmd, VkQueue queue, bool waitForExecution)
{
  nvvk_ctx_t* vkctx = driver->vkctx;
...
void
lr::nv_vk_stage_image_transfer(nvvk_driver* driver, VkImage dst, const void* data, size_t width, size_t height, size_t image_size)
{
  nvvk_ctx_t* vkctx = driver->vkctx;
  ...
  const VkMemoryAllocateInfo stagingBufferAllocInfo = {
    ...
    .memoryTypeIndex = lr::nv_vk_get_mem_type(vkctx, stagingBufferRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)
  };
  ...
  const VkCommandBuffer cmd = lr::nv_vk_begin_command_buffer(driver);

  lr::nv_vk_insert_texture_layout_transition(
      cmd,
      dst,
      1,
  ...
  lr::nv_vk_end_command_buffer(driver, cmd, vkctx->transfer_queue, true);
...
void
lr::nv_vk_create_texture_from_memory(nvvk_driver* driver, u8* buffer, u32 width, u32 height, nv_format format, VkImage* dst, VkDeviceMemory* dstMem)
{
  lr::nv_vk_create_texture_empty(driver->vkctx, width, height, format, VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, NULL, dst, dstMem);
  lr::nv_vk_stage_image_transfer(driver, *dst, buffer, width, height, width * height * lr::nv_format_get_bytes_per_pixel(format));
}

// If the format given is oki then it'll just return it
// otherwise it gives you the next best optoin
nv_format
lr::nv_vk_get_supported_format_for_draw(nvvk_ctx_t* vkctx, nv_format fmt)
{
  VkFormatProperties formatProperties;
  vkGetPhysicalDeviceFormatProperties(vkctx->phys_device, (VkFormat)lr::nv_format_to_vk_format(fmt), &formatProperties);

  if ((formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT) == 0)
  {
    // format not supported
    // TODO: Make something that selects a format with the same number of channels as the format needed.
    nv_log_warning("Format %i (%s) unsupported. Using NOVA_FORMAT_RGBA8\n", fmt, lr::nv_format_to_string(fmt));
    ...
    VkDeviceMemory*       dstMem)
{
  ...
  {
    format = lr::nv_vk_get_supported_format_for_draw(vkctx, format);
  }
  ...
  imageCreateInfo.format            = (VkFormat)lr::nv_format_to_vk_format(format);
  ...
  if (dstMem != NULL)
  {
    const u32 localDeviceMemoryIndex = lr::nv_vk_get_mem_type(vkctx, imageMemoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
u8*
lr::nv_vk_create_texture_from_disk(nvvk_driver* driver, const char* path, u32* width, u32* height, nv_format* channels, VkImage* dst, VkDeviceMemory* dstMem)
{
  ...
  *channels = tex.format;
  ...
  lr::nv_vk_create_texture_from_memory(driver, tex.data, tex.width, tex.height, *channels, dst, dstMem);
  return tex.data;
}
...
bool
lr::nv_vk_get_supported_format(nvvk_ctx_t* vkctx, VkPhysicalDevice phys_device, VkSurfaceKHR surface, nv_format* dst_format, VkColorSpaceKHR* dst_color_space)
{
  ...
  {
    *dst_format      = lr::nv_vk_format_to_nv_format(selected_format.format);
...
u32
lr::nv_vk_get_surface_image_count(nvvk_ctx_t* vkctx, VkPhysicalDevice phys_device, VkSurfaceKHR surface)
{
  NVVK_REQUIRED_PTR(phys_device);
  NVVK_REQUIRED_PTR(surface);
...
void
nv_gpu_allocate_memory(nvvk_ctx_t* vkctx, size_t size, nv_gpu_memory_usage usage, nv_gpu_memory_t* dst)
{
  ...
  vkGetPhysicalDeviceMemoryProperties(vkctx->phys_device, &mem_properties);
  ...
  nvvk_result_check(*vkctx, vkAllocateMemory(vkctx->device, &alloc_info, &vkctx->vkalloc, &dst->memory));
}

void
nv_gpu_map_memory(nvvk_ctx_t* vkctx, nv_gpu_memory_t* memory, VkDeviceSize size, size_t offset, void** out)
{
  ...
  {
    nv_log_error(
        "Memory usage does not have NOVA_GPU_MEMORY_USAGE_CPU_VISIBLE \n"
  ...
  if (vkMapMemory(vkctx->device, memory->memory, offset, size, 0, &memory->mapped) != VK_SUCCESS)
  {
...
void
nv_gpu_unmap_memory(nvvk_ctx_t* vkctx, nv_gpu_memory_t* memory)
{
  ...
  if (!(memory->usage & NOVA_GPU_MEMORY_USAGE_CPU_COHERENT))
  {
    ...
    vkFlushMappedMemoryRanges(vkctx->device, 1, &range);
  }
  memory->map_offset = 0;
  memory->map_size   = 0;
  memory->mapped     = NULL;
  vkUnmapMemory(vkctx->device, memory->memory);
}

void
nv_gpu_free_memory(nvvk_ctx_t* vkctx, nv_gpu_memory_t* mem)
{
  if (mem && mem->memory)
  {
    vkFreeMemory(vkctx->device, mem->memory, NULL);
  }
}

void
nv_gpu_copy_memory(nvvk_ctx_t* vkctx, nv_gpu_memory_t* dst, nv_gpu_memory_t* src, size_t size, size_t dst_offset, size_t src_offset)
{
  nv_assert_else_return(dst != NULL, );
  ...
  if (src->mapped)
  {
    nv_gpu_unmap_memory(vkctx, src);
  }

  if (dst->mapped)
  {
    nv_gpu_unmap_memory(vkctx, dst);
  }

  void* dst_mapped = NULL;
  void* src_mapped = NULL;
  nv_gpu_map_memory(vkctx, dst, size, dst_offset, &dst_mapped);
  nv_gpu_map_memory(vkctx, src, size, src_offset, &dst_mapped);
...
// void
// nv_gpu_bind_buffer_to_memory(nvvk_ctx_t* vkctx, nv_gpu_memory_t* mem, size_t offset, nv_gpu_buffer_t* buffer)
// {
//   buffer->memory = mem;
//   buffer->offset = offset;

//   nvvk_result_check(*vkctx, vkBindBufferMemory(vkctx->device, buffer->buffer, mem->memory, offset));
// }
...
void
nv_gpu_bind_texture_to_memory(nvvk_ctx_t* vkctx, nv_gpu_memory_t* mem, size_t offset, nv_gpu_texture* tex)
{
  tex->memory = mem;
  tex->offset = offset;
  nvvk_result_check(*vkctx, vkBindImageMemory(vkctx->device, tex->image, mem->memory, offset));

  VkImageAspectFlags aspect = 0;
  if (lr::nv_format_has_depth_channel(tex->format))
  {
    aspect = VK_IMAGE_ASPECT_DEPTH_BIT;
  }
  else if (lr::nv_format_has_color_channel(tex->format))
  {
    aspect = VK_IMAGE_ASPECT_COLOR_BIT;
  }

  if (lr::nv_format_has_stencil_channel(tex->format))
  {
  ...
  dst_format           = lr::nv_vk_get_supported_format_for_draw(vkctx, dst_format);

  VkImageViewCreateInfo view_info = {
    .sType            = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
    .image            = tex->image,
    .viewType         = (VkImageViewType)tex->type,
    .format           = (VkFormat)lr::nv_format_to_vk_format(dst_format),
    .components       = { VK_COMPONENT_SWIZZLE_IDENTITY, VK_COMPONENT_SWIZZLE_IDENTITY, VK_COMPONENT_SWIZZLE_IDENTITY, VK_COMPONENT_SWIZZLE_IDENTITY },
    .subresourceRange = subresourceRange,
  };
  nvvk_result_check(*vkctx, vkCreateImageView(vkctx->device, &view_info, &vkctx->vkalloc, &tex->view));
}

void
nv_gpu_destroy_texture(nvvk_ctx_t* vkctx, nv_gpu_texture* tex)
{
  vkDeviceWaitIdle(vkctx->device);
  if (!tex->view)
  {
  ...
  vkDestroyImage(vkctx->device, tex->image, &vkctx->vkalloc);
  vkDestroyImageView(vkctx->device, tex->view, &vkctx->vkalloc);
}
...
// void
// nv_gpu_buffer_readback(nvvk_ctx_t* vkctx, const nv_gpu_buffer_t* buffer, void* dst)
// {
...
//   nv_gpu_create_buffer(vkctx, buffer->size, NOVA_GPU_ALIGNMENT_UNNECESSARY, NOVA_GPU_BUFFER_USAGE_TRANSFER_DESTINATION, &staging);
//   nv_gpu_allocate_memory(vkctx, buffer->size, NOVA_GPU_MEMORY_USAGE_CPU_VISIBLE, &staging_mem);
//   nv_gpu_bind_buffer_to_memory(vkctx, &staging_mem, 0, &staging);

//   VkCommandBuffer cmd = lr::nv_vk_begin_command_buffer(vkctx);

//   VkBufferCopy copy = { .srcOffset = 0, .dstOffset = 0, .size = buffer->size };
//   vkCmdCopyBuffer(cmd, buffer->buffer, staging.buffer, 1, &copy);

//   lr::nv_vk_end_command_buffer(vkctx, cmd, vkctx->transfer_queue, 1);

//   void* mapped = NULL;
//   nv_gpu_map_memory(vkctx, &staging_mem, buffer->size, 0, &mapped);
//   nv_assert(mapped != NULL);
//   nv_memcpy(dst, mapped, buffer->size);
//   nv_gpu_unmap_memory(vkctx, &staging_mem);

//   nv_gpu_destroy_buffer(vkctx, &staging);
//   nv_gpu_free_memory(vkctx, &staging_mem);
// }
...
void
nv_gpu_create_sampler(nvvk_driver* driver, const nv_gpu_sampler_create_info* pInfo, nv_gpu_sampler_t** dst)
{
  ...
  *dst = new_sampler;
  ...
  nvvk_result_check(*driver->vkctx, vkCreateSampler(driver->vkctx->device, &samplerInfo, &driver->vkctx->vkalloc, &new_sampler->vksampler));
}

void
nv_gpu_write_to_texture(nvvk_driver* driver, nv_gpu_texture* tex, const nv_image* src)
{
  if (!(tex->usage & VK_IMAGE_USAGE_TRANSFER_DST_BIT))
  {
    nv_log_error(
        "Cannot write to an image which does not have usage \n"
        "VK_IMAGE_USAGE_TRANSFER_DST_BIT");
  }

  const size_t tex_size = tex->extent.width * tex->extent.height * lr::nv_format_get_bytes_per_pixel(tex->format);
  ...
  lr::nv_vk_stage_image_transfer(driver, tex->image, src->data, src->width, src->height, tex_size);
}
...
void
nv_gpu_create_texture(nvvk_ctx_t* vkctx, const nv_gpu_texture_create_info* pInfo, nv_gpu_texture* dst)
{
  nv_assert_else_return(pInfo != NULL, );
  ...
  if ((pInfo->usage & VK_IMAGE_USAGE_SAMPLED_BIT) != 0)
  {
    fmt = lr::nv_vk_get_supported_format_for_draw(vkctx, fmt);
  }

  dst->type   = (VkImageType)pInfo->type;
  dst->format = pInfo->format;
  dst->extent = (VkExtent3D){ (u32)pInfo->extent.width, (u32)pInfo->extent.height, (u32)pInfo->extent.depth };
  ...
  VkImageCreateInfo imageCreateInfo = nv_zero_init(VkImageCreateInfo);
  imageCreateInfo.sType             = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
  imageCreateInfo.imageType         = (VkImageType)pInfo->type;
  imageCreateInfo.extent            = (VkExtent3D){ (u32)pInfo->extent.width, (u32)pInfo->extent.height, (u32)pInfo->extent.depth };
  imageCreateInfo.mipLevels         = pInfo->miplevels;
  imageCreateInfo.arrayLayers       = pInfo->arraylayers;
  imageCreateInfo.format            = (VkFormat)lr::nv_format_to_vk_format(fmt);
  imageCreateInfo.tiling            = VK_IMAGE_TILING_OPTIMAL;
  imageCreateInfo.initialLayout     = VK_IMAGE_LAYOUT_UNDEFINED;
  imageCreateInfo.usage             = usage;
  imageCreateInfo.samples           = (VkSampleCountFlagBits)pInfo->samples;
  imageCreateInfo.sharingMode       = VK_SHARING_MODE_EXCLUSIVE;
  nvvk_result_check(*vkctx, vkCreateImage(vkctx->device, &imageCreateInfo, &vkctx->vkalloc, &dst->image));
}

uint32_t
lr::nv_format_to_vk_format(nv_format format)
{
  switch (format)
  {
...
nv_format
lr::nv_vk_format_to_nv_format(VkFormat_ format)
{
  switch (format)
  {
...
nv_error
nv_sprite_load_from_memory(nvvk_driver* driver, const unsigned char* data, size_t w, size_t h, nv_format fmt, nv_sprite_t* dst)
{
  ...
  fmt      = lr::nv_vk_get_supported_format_for_draw(driver->vkctx, fmt);
  dst->fmt = fmt;
  ...
  nv_gpu_create_texture(driver->vkctx, &tex_info, &dst->tex);

  VkMemoryRequirements mem_req;
  vkGetImageMemoryRequirements(driver->vkctx->device, nv_gpu_texture_get(&dst->tex), &mem_req);

  nv_gpu_allocate_memory(driver->vkctx, mem_req.size, NOVA_GPU_MEMORY_USAGE_CPU_VISIBLE, &dst->mem);
  nv_gpu_bind_texture_to_memory(driver->vkctx, &dst->mem, 0, &dst->tex);

  const nv_image img = (const nv_image){ .width = w, .height = h, .format = fmt, .data = (unsigned char*)data };
...
nv_error
nv_sprite_load_from_disk(nvvk_driver* driver, const char* path, nv_sprite_t* dst)
{
  nv_assert_else_return(driver != NULL, NV_ERROR_INVALID_ARG);
  nv_assert_else_return(nvvk_ctx_is_valid(driver->vkctx) == true, NV_ERROR_INVALID_ARG);
...
nv_sprite_destroy(nvvk_ctx_t* vkctx, nv_sprite_t* spr)
{
  ...
  nv_gpu_destroy_texture(vkctx, &spr->tex);
  nv_gpu_free_memory(vkctx, &spr->mem);
}
...
void
nv_camera_destroy(nvvk_ctx_t* vkctx, nv_camera_t* cam)
{
  // nv_descriptor_set_t_destroy(cam->sets);
  nv_gpu_buffer_destroy(&cam->ub);
  nv_gpu_memory_pool_free(&cam->mem);
}

nv_error
nv_camera_init(nvvk_driver* driver, nv_camera_t* cam)
{
  ...
  *cam = (nv_camera_t){
  ...
  // nv_error code = nv_gpu_memory_pool_allocate(&driver->cpu_mappable_pool, ub_size * CAMERA_FAKE_BUFFER_COUNT, 8, &cam->mem);
  // nv_assert_else_return(code == NV_SUCCESS, code);

  nv_error code = nv_gpu_memory_allocate_dedicated(driver, NV_GPU_MEMORY_PERSISTENT_MAPPED_BIT, ub_size * CAMERA_FAKE_BUFFER_COUNT, 8, &cam->mem);
  nv_assert_else_return(code == NV_SUCCESS, code);

  // nv_gpu_create_buffer(vkctx, ub_size * CAMERA_FAKE_BUFFER_COUNT, ub_align, NOVA_GPU_BUFFER_USAGE_UNIFORM_BUFFER, &cam->ub);
  code = nv_gpu_buffer_init(driver, &cam->mem, ub_size * CAMERA_FAKE_BUFFER_COUNT, ub_align, NV_GPU_BUFFER_UNIFORM_BUFFER_BIT, &cam->ub);
  nv_assert_else_return(code == NV_SUCCESS, code);

  VkDescriptorSetLayoutBinding bindings[] = { { 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, 1, VK_SHADER_STAGE_VERTEX_BIT, NULL } };
  if (nv_allocate_descriptor_set(vkctx, &g_pool, bindings, 1, &cam->sets) != NV_ERROR_SUCCESS)
  {
    return NV_ERROR_INVALID_RETVAL;
  }
  ...
  if (nv_descriptor_set_submit_write(vkctx, cam->sets, &write) != NV_ERROR_SUCCESS)
  {
    return NV_ERROR_INVALID_RETVAL;
  }

  nv_gpu_memory_map(cam->ub.block, 0, ub_size * CAMERA_FAKE_BUFFER_COUNT, (void**)&cam->mem_mapped);
...
nv_camera_update(nv_camera_t* cam, struct nv_renderer* rd)
{
  ...
  const size_t index     = nv_camera_get_read_offset(&camera) / sizeof(nv_camera_uniform_buffer_t);
  cam->mem_mapped[index] = ub;
}
...
const char*
lr::nvvk_vk_result_to_string(VkResult r)
{
  switch (r)
  {
...
int
nv_gpu_create_framebuffer(nvvk_ctx_t* vkctx, const nv_gpu_framebuffer_create_info_t* pCreateInfo, nv_gpu_framebuffer_t* dst)
{
  ...
  framebufferInfo.attachmentCount         = (u32)nv_list_size(&attachments);
...
void
nv_gpu_destroy_framebuffer(nvvk_ctx_t* vkctx, nv_gpu_framebuffer_t* dst)
{
  if (!dst)
  {
    return;
