...
#include "GPU/shaderresource.hpp"
...
nv_error _nvsm_load_list_file(lr::nvsm_ctx_t* ctx, lr::nvsm_list_file_t* file);

/* returns NV_ERROR_FILE_NOT_FOUND to indicate the file was not found. This should be handled by the dev appropriately. */
nv_error _nvsm_load_cache_file(lr::nvsm_ctx_t* ctx, lr::nvsm_cache_file_t* file);

/* write the cache to disk */
nv_error _nvsm_generate_and_write_cache_file(const lr::nvsm_list_file_t* file, size_t list_file_mtime, FILE* out_file);
...
static inline lr::nvsm_compile_options_t
_nvsm_get_default_compile_options(void)
{
  lr::nvsm_compile_options_t opts = nv_zero_init(lr::nvsm_compile_options_t);
...
nv_error
_nvsm_load_list_file(lr::nvsm_ctx_t* ctx, lr::nvsm_list_file_t* file)
{
  ...
  file->entries = (lr::nvsm_list_file_entry_t*)nv_calloc(num_lines_list_file * sizeof(lr::nvsm_list_file_entry_t));
  ...
  {
    lr::nvsm_list_file_entry_t* entry = &file->entries[idx];
...
nv_error
_nvsm_load_cache_file(lr::nvsm_ctx_t* ctx, lr::nvsm_cache_file_t* file)
{
  nv_assert_else_return(ctx != NULL, NV_ERROR_INVALID_ARG);
  nv_assert_else_return(file != NULL, NV_ERROR_INVALID_ARG);
  ...
  file->entries = (lr::nvsm_cache_file_entry_t*)nv_calloc(file->num_entries * sizeof(lr::nvsm_cache_file_entry_t));
  ...
  if (fread(file->entries, sizeof(lr::nvsm_cache_file_entry_t), file->num_entries, cache_file) != file->num_entries)
  {
    fclose(cache_file);
    return NV_ERROR_INVALID_CACHE;
  }
...
static inline void
_nvsm_destroy_cache_file(lr::nvsm_cache_file_t* file)
{
  if (file->entries)
  {
...
static inline void
_nvsm_destroy_list_file(lr::nvsm_list_file_t* file)
{
  if (file->entries)
  {
    nv_assert_else_return(file->num_entries != 0, );
...
nv_error
_nvsm_generate_and_write_cache_file(const lr::nvsm_list_file_t* file, size_t list_file_mtime, FILE* out_file)
{
  nv_assert_else_return(file != NULL, NV_ERROR_INVALID_ARG);
  ...
  lr::nvsm_cache_file_entry_t* cache_converted_entries = (lr::nvsm_cache_file_entry_t*)nv_calloc(sizeof(lr::nvsm_cache_file_entry_t) * file->num_entries);
  nv_assert_else_return(cache_converted_entries != NULL, NV_ERROR_MALLOC_FAILED);

  for (size_t idx = 0; idx < file->num_entries; idx++)
  {
    const lr::nvsm_list_file_entry_t* entry   = &file->entries[idx];
    lr::nvsm_cache_file_entry_t*      convert = &cache_converted_entries[idx];

    *convert = nv_zero_init(lr::nvsm_cache_file_entry_t);
  ...
  if (fwrite(cache_converted_entries, sizeof(lr::nvsm_cache_file_entry_t), file->num_entries, out_file) != file->num_entries)
  {
    return NV_ERROR_IO_ERROR;
  }
...
_nvsm_dump_shader(const lr::nvsm_spirv_binary_t* bin, const char* out_filename)
{
...
static inline nv_error
_nvsm_compile_shader(const char* shader_path, const lr::nvsm_compile_options_t* opts, const char* spirv_path, const char stage[4], lr::nvsm_spirv_binary_t* bin, bool dump)
{
  ...
  uchar             buffer[16384];
  nv_alloc_estack_t stack = (nv_alloc_estack_t){
    .buffer      = buffer,
  ...
  *bin = nv_zero_init(lr::nvsm_spirv_binary_t);
...
static inline nv_error
_nvsm_default_compile_no_cache(lr::nvsm_ctx_t* ctx, lr::nvsm_list_file_t* list_file)
{
  lr::nvsm_compile_options_t compile_options = _nvsm_get_default_compile_options();
  ...
  {
    lr::nvsm_list_file_entry_t* list_entry = &list_file->entries[list_i];
...
static inline nv_error
_nvsm_default_compile_with_cache(lr::nvsm_ctx_t* ctx, lr::nvsm_list_file_t* list_file, lr::nvsm_cache_file_t* cache_file)
{
  nv_error code = NV_SUCCESS;
  ...
  for (size_t list_i = 0; list_i < list_file->num_entries; list_i++)
  {
    ...
    */
    ...
    {
      const lr::nvsm_cache_file_entry_t* cache_entry = &cache_file->entries[cache_i];
      if (hash_name == cache_entry->hash_name && hash_shader_path == cache_entry->hash_shader_path)
      {
        ...
        nv_bzero(&list_entry->bin, sizeof(lr::nvsm_spirv_binary_t));
 ...
 */
static inline nv_error
_generate_and_dump_cache_file(const char* cache_file_dir, const lr::nvsm_list_file_t* list_file, size_t list_file_last_modtime)
{
  /**
   * This is legal
...
nv_error
lr::nvsm_compile_shaders(lr::nvsm_ctx_t* ctx)
{
  nv_assert_else_return(ctx != NULL, NV_ERROR_INVALID_ARG);
  ...
  lr::nvsm_list_file_t  list_file  = nv_zero_init(lr::nvsm_list_file_t);
  lr::nvsm_cache_file_t cache_file = nv_zero_init(lr::nvsm_cache_file_t);
...
nv_error
lr::nvsm_create_shader_modules(nvvk_ctx_t* vkctx, lr::nvsm_ctx_t* ctx)
{
  nv_assert_else_return(ctx != NULL, NV_ERROR_INVALID_ARG);
  ...
  {
    lr::nvsm_list_file_entry_t* entry = (lr::nvsm_list_file_entry_t*)node->value;
    if (!entry)
    {
      continue;
    }
    ...
    entry->resources = lr::shader_resources::extract(entry->bin.words, entry->bin.byte_count, &entry->num_resources);
...
nv_error
lr::nvsm_load_shader(lr::nvsm_ctx_t* ctx, const char* name, nvsm_shader_t** out)
{
...
nv_error
lr::nvsm_compile_shaders_force(lr::nvsm_ctx_t* ctx, bool generate_cache)
{
  nv_assert_else_return(ctx != NULL, NV_ERROR_INVALID_ARG);
...
VkShaderStageFlags
lr::_nvsm_shader_stage_from_string(const char stage[4])
{
  if (nv_strncmp(stage, "vert", 4) == 0)
  {
...
nv_error
lr::nvsm_init(lr::nvsm_ctx_t* ctx)
{
  nv_assert_else_return(ctx != NULL, NV_ERROR_INVALID_ARG);

  nv_bzero(ctx, sizeof(lr::nvsm_ctx_t));

  nv_error code = nv_hashmap_init(16, sizeof(const char*), sizeof(lr::nvsm_list_file_entry_t), nv_hash_fnv1a_string, nv_allocator_c, NULL, &ctx->shader_map);
  if (code != NV_SUCCESS)
  {
    return code;
  }
...
void
lr::nvsm_shutdown(nvvk_ctx_t* vkctx, lr::nvsm_ctx_t* ctx)
{
  ...
  while ((node = nv_hashmap_iterate_unsafe(&ctx->shader_map, &_hashmap_iter)) != NULL)
  {
    ...
    if (entry->resources != NULL && entry->num_resources > 0)
    {
      ...
      {
        lr::shader_resources::resources_t* resource = &entry->resources[i];
...
lr::shader_resources::resource_type
lr::shader_resources::_shader_resources_type_spv_reflect_type(SpvReflectDescriptorType_ type)
{
  switch (type)
  {
    case SPV_REFLECT_DESCRIPTOR_TYPE_UNIFORM_BUFFER: return lr::shader_resources::NV_SHADER_RESOURCE_TYPE_UNIFORM_BUFFER;
    case SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_BUFFER: return lr::shader_resources::NV_SHADER_RESOURCE_TYPE_STORAGE_BUFFER;
    case SPV_REFLECT_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER: return lr::shader_resources::NV_SHADER_RESOURCE_TYPE_COMBINED_IMAGE_SAMPLER;
    case SPV_REFLECT_DESCRIPTOR_TYPE_SAMPLED_IMAGE: return lr::shader_resources::NV_SHADER_RESOURCE_TYPE_SAMPLED_IMAGE;
    case SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_IMAGE: return lr::shader_resources::NV_SHADER_RESOURCE_TYPE_STORAGE_IMAGE;
    case SPV_REFLECT_DESCRIPTOR_TYPE_SAMPLER: return lr::shader_resources::NV_SHADER_RESOURCE_TYPE_SAMPLER;
    default: return lr::shader_resources::NV_SHADER_RESOURCE_TYPE_UNKNOWN;
  }
}

lr::shader_resources::resources_t*
lr::shader_resources::extract(const u32* spirv_data, size_t spirv_size, size_t* out_count)
{
  ...
  SpvReflectShaderModule handle;
  SpvReflectResult       result = spvReflectCreateShaderModule(spirv_size, spirv_data, &handle);
  ...
  u32 binding_count = 0;
  spvReflectEnumerateDescriptorBindings(&handle, &binding_count, NULL);

  if (binding_count == 0)
  {
    spvReflectDestroyShaderModule(&handle);
    *out_count = 0;
    return NULL;
  }
  ...
  resources_t* resources = (resources_t*)nv_malloc(sizeof(resources_t) * binding_count);
  nv_memset(resources, 0, sizeof(resources_t) * binding_count);
  ...
  for (size_t i = 0; i < binding_count; i++)
  {
    ...
    resources[i].type         = _shader_resources_type_spv_reflect_type(binding->descriptor_type);
