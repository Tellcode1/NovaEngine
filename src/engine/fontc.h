#ifndef NOVAENGINE_SRC_ENGINE_FONTC_H
#define NOVAENGINE_SRC_ENGINE_FONTC_H
...
using fontc_file_t        = struct fontc_file_t;
using fontc_file_header_t = struct fontc_file_header_t;
using fontc_glyph_t       = struct fontc_glyph_t;

using fontc_err_t = enum fontc_err_t {
  FONTC_SUCCESS                           = 0,
...
extern auto fontc_read_font(const char* path, fontc_file_t* file) -> fontc_err_t;
extern auto fontc_bake_font_to_cache(const char* font_path, size_t pixel_size, size_t init_atlas_w, size_t init_atlas_h, size_t num_threads, fontc_file_t* out_file)
    -> fontc_err_t;
extern auto fontc_write_font_file(const char* out, fontc_file_t* file) -> fontc_err_t;
 ...
 */
extern auto fontc_load_font(const char* font_source_path, size_t pixel_size, fontc_file_t* f_file) -> fontc_err_t;

NOVA_HEADER_END

#endif // NOVAENGINE_SRC_ENGINE_FONTC_H
