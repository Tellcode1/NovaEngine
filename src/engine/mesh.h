...
{
  nv_gpu_create_buffer(sizeof(nv_vertex_t) * nvertices + sizeof(u32) * nindices, 1, NOVA_GPU_BUFFER_USAGE_VERTEX_BUFFER | NOVA_GPU_BUFFER_USAGE_INDEX_BUFFER, &dst->buf);
  nv_gpu_allocate_memory(
      sizeof(nv_vertex_t) * nvertices + sizeof(u32) * nindices,
      NOVA_GPU_MEMORY_USAGE_GPU_LOCAL | NOVA_GPU_MEMORY_USAGE_CPU_VISIBLE | NOVA_GPU_MEMORY_USAGE_CPU_COHERENT,
      &dst->mem);
...
nv_mesh_render(nv_renderer_t* rd, nv_sprite* spr, nv_mesh_t* mesh)
{
  ...
  pc.light_position = (vec3f){ camera.position.x, camera.position.y, camera.position.z };
  pc.view_position  = (vec3f){ camera.position.x, camera.position.y, camera.position.z };
