...
nv_texture_atlas_init(size_t width, size_t height, nv_format fmt, u32 padding, nv_texture_atlas_t* dst)
{
  ...
  *dst = nv_zinit(nv_texture_atlas_t);
  ...
  dst->format  = fmt;
  dst->padding = padding;
  dst->data    = (unsigned char*)nv_zmalloc(width * height * nv_format_get_bytes_per_pixel(dst->format));
...
nv_texture_atlas_resize(nv_texture_atlas_t* atlas, int scale)
{
  ...
  unsigned char* new_data = (uchar*)nv_zmalloc(new_w * new_h * channels);
...
nv_texture_atlas_finish(nv_texture_atlas_t* atlas)
{
  ...
  if (atlas->width > optimal_w || atlas->height > optimal_h)
  {
    ...
    unsigned char* new_data = (unsigned char*)nv_zmalloc(max_w * max_h * channels);
    if (new_data != NULL)
    {
